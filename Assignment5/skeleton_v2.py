...
def stochast_train_w(x_train,y_train,learn_rate=0.1,niter=1000):
    ...
    for it in xrange(niter):
        ...
        y=y_train[xy_index]
        for i in xrange(dim):
            update_grad = loss_func_deriv(w, x, y) ### something needs to be done here
...
def loss_func_deriv(w, x, y):
    return (y - logistic_wx(w, x)) * x * np.exp(-np.dot(w,x)) / ( 1 + np.exp(- np.inner(w, x)))**2
